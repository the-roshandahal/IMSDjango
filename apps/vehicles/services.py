...
@transaction.atomic
def assign(
    *, vehicle_id, performed_by, location=None, project_id=None, driver_id=None,
    comment="", override=False, override_reason="",
):
    """Assign to a general location (Deep Clean / Car Park / Other) or a
    specific deep clean project (mutually exclusive) -- the latter is more
    precise and drives per-project cost tracking."""
    if location is not None and project_id is not None:
        raise ValueError("Assign to a location or a project, not both.")
    if location is None and project_id is None:
        raise ValueError("Assign requires a location or project_id.")
    ...
    updated = Vehicle.objects.filter(pk=vehicle_id, status=VehicleStatus.AVAILABLE).update(
        status=VehicleStatus.ASSIGNED, current_location=location or "", current_project_id=project_id,
    ...
    VehicleLog.objects.create(
        vehicle_id=vehicle_id, action="assigned", location=location or "", project_id=project_id, driver_id=driver_id,
...
# Backward-compatible alias -- general-location assignment is still the common case.
def assign_to_location(*, vehicle_id, location, driver_id, performed_by, comment="", override=False, override_reason=""):
    return assign(
        vehicle_id=vehicle_id, location=location, driver_id=driver_id, performed_by=performed_by,
...
def release(*, vehicle_id, performed_by, comment=""):
    updated = Vehicle.objects.filter(pk=vehicle_id, status=VehicleStatus.ASSIGNED).update(
        status=VehicleStatus.AVAILABLE, current_location="", current_project=None, assigned_driver=None,
    )
    if updated == 0:
...
def start_maintenance(*, vehicle_id, performed_by, comment=""):
    updated = (
        ...
        .update(status=VehicleStatus.IN_MAINTENANCE, current_location="", current_project=None, assigned_driver=None)
    )
    if updated == 0:
...
def write_off(*, vehicle_id, performed_by, comment=""):
    updated = (
        ...
        .update(status=VehicleStatus.WRITTEN_OFF, current_location="", current_project=None, assigned_driver=None)
    )
    if updated == 0:
...
def log_cost(*, vehicle_id, cost_type, amount, incurred_at, recorded_by, comment="", location=None, project_id=None):
    ...
    return VehicleCostLog.objects.create(
        ...
        location=location if location is not None else vehicle.current_location,
