...
from apps.vehicles.models import CostType, Vehicle, VehicleCostLog
...
class VehicleSerializer(serializers.ModelSerializer):
    class Meta:
        model = Vehicle
        fields = [
            "id", "registration", "make_model", "status", "current_station", "assigned_driver",
            "service_due_date", "insurance_expiry", "created_at",
        ]
        read_only_fields = ["id", "status", "current_station", "assigned_driver", "created_at"]


class VehicleCostLogSerializer(serializers.ModelSerializer):
    class Meta:
        model = VehicleCostLog
        fields = ["id", "vehicle", "cost_type", "amount", "incurred_at", "station", "comment", "recorded_by", "recorded_at"]
